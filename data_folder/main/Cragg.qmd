...
There are tons of warnings from packages masking and such. So, in the YAML I have suppressed warnings.
...
```{r}
#| eval: false
#This also isn't working. Let's just try the OLS first
mhurdle(data = replic,
        b ~ quota_remaining_BOQ + proportion_observed + quarter | live_priceGDP,
        h2 = TRUE)
```

```{r}
#| eval: false
#This does not work. Trying a simpler model to understand where I went wrong
mhurdle(data = replic, 
        b ~ quota_remaining_BOQ + fraction_remaining_BOQ + proportion_observed + quarter | live_priceGDP + quota_remaining_BOQ + fraction_remaining_BOQ + WTswt_quota_remaining_BOQ + WTDswt_quota_remaining_BOQ + quarter,
        h2 = TRUE,
        dist = "ln",
        corr = TRUE,
        method = "bhhh")
