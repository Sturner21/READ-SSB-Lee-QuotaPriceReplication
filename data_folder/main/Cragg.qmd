...
```{r}
#model 1a makes the linear hurdle model that Lee & D made
#model 1b makes the exponential hurdle model that Lee & D made

#MAKE SURE STARS ARE AT PROPER SIGNIFICANCE LEVEL AS WE WOULD EXPECT - HAD TO CHANGE THESE
...
We should add in the time fixed effects of time that Lee & D left out for some reason. See if that changes any of the significance that I have
...
```{r}
modelsummary(models = list("1aa" = hurdle.1aa, "1ab" = hurdle.1ab),
             stars = c('*' = .1, '**' = .05, '***' = 0.01))
```
...
Actually, can't log these because log(0)=-Inf which is an error.

```{r}
#Good looking display but need to find out how to make AIC and BIC display somehow
...
We can take some of the climate change variables (namely SST and BST) that we use above and create dummy variables for them to see if they are in the 90th percentile. This should mean that in the years where we see extreme heat deviations that we see changes in the behavior of stocks. This should lead to this dummy variable having a statistically significant and positive impact on the quota price.
...
Now that we have added the dummy variable columns we can add it to the respective regressions.
...
Now that we have coded those dummy variables we can run some models to see how these adjustments affect quota prices.

```{r}
#This creates the OLS which has a time fixed effect and controls for all climate change variables as well as replicating the other independent variables from earlier sections of replication
...
Interestingly, there is no difference in the OLS model after coding it in. And, we can see too that this doesn't affect the significance of the model's predictors, nor the model's explanatory power.

```{r}
#This creates the full fixed effects model, controlling for both time and stock (species) fixed effects as well as cliamte and other earlier independent variables from replication
...
Even after accounting for different stock stocks we still don't see the stock adjustment making an impact.

```{r}
#This is a "messy" model output since I have not removed the statistically insignificant variables from the model

#This creates the summary of the two above models
#Not included is the stock_id dummy variables but all 20 are significant to the 0.05 level except for George's Bank West Cod, Winter Flounder, Cape Cod/Gulf of Maine Yellow(tail?) Flounder, Southern New England Yellow(tail?) flounder
...
#Made the eval false becuase I've indluded this in the table below with the churdle.5*
modelsummary(models = list("Robust Non-Correlated Expoenetial Climate" = churdle.4a),
             stars = c('*' = .05, '**' = .01, '***' = 0.001),
             coef_rename = c("h1.(Intercept)" = "Intercept [H1]",
                             "h1.quota_remaining_BOQ" = "Quota Remaining [H1]",
                             "h1.fraction_remaining_BOQ" = "Fraction Quota Remaining [H1]",
...
```{r}
#Both c5a and c5b are exponential models which include stock adjustments, 90th percentile for temperature dummy, correlation of errors check, and climate change variables
#The difference between these first two is that c5a has robust errors and c5b is not
#The third model, c5c, is a linear hurdle model but has the same characteristics as c5a and c5b

modelsummary(models = list("Robust Correlated Exponential Climate" = churdle.5a, "Non-Robust Correlated Exponential Climate" = churdle.5b, "Robust Correlated Linear Climate" = churdle.5c, "Robust Non-Correlated Expoenetial Climate" = churdle.4a),
...
Interestingly, neither the stock readjustment nor the 90th percentile of BST seem to help improve the model when considering correlation between the models. When not considering correlation they actually do help the model's explanatory power.
